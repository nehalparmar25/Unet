...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep 14 12:00:29 2018
...
def transform_mask(mask):
    ...
    #cols, rows = mask.size
    #cols, rows, channels = mask.shape
    #img2 = Image(mask)#Image.open(mask).convert('RGB')
    #img2 = cv2.imread(mask)

    #cols, rows = img2.size

    new_mask = np.zeros([rows , cols, 3])
    mask = np.array(mask)
    mask.reshape(rows,cols,3)
    flag=0;
#    a = np.shape(mask)
#    print(a)
    #rows=rows-1
...
def trainGenerator(batch_size,train_path,image_folder,mask_folder,aug_dict,image_color_mode = "rgb",
   ...
   # print(image_generator)
    ...
    #Converting each mask into binary mode
    images = []
    folder = train_path + mask_folder
    print(folder)
    i = 21
    for filename in os.listdir(folder):
            img = cv2.imread(os.path.join(folder,filename))
            if img is not None:
                img1 = transform_mask(img)
                #print(type(img1))
                #img2 = Image.fromarray(img1)
                img2 = Image.fromarray(img1.astype('uint8'))
                p = '/home/lenovo/Documents/Major Project/Project material/Dataset/Retinal/AV - DRIVE/AV_DRIVE_groundtruth/training/modified_mask/'
                imgname = str(i) + '_training.png'
                complete_path = p + imgname
                img2.save(complete_path)
                i=i+1
    mask_folder = 'modified_mask'
